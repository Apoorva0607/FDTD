...
if S1 ==1  % gaussian wave
    ...
    nmax = 35000; % found by plotting the gaussian function with time
...
% coefficients
%(Ca and Cb defined for point i is not relevant an is ignored in Ez calculation)
for i = 1:imax
...
figure(3);
plot(relative_error,'LineWidth', 2);
title('Error due to PML', 'LineWidth', 14);
xlabel('Time steps(n)', 'LineWidth', 14);
ylabel('Relative Error', 'LineWidth', 14);

figure(4);
semilogy(relative_error, 'LineWidth',2);
title('log Error due to PML', 'LineWidth', 14);
xlabel('Time steps(n)', 'LineWidth', 14);
ylabel('Relative Error', 'LineWidth', 14);

figure(5);
plot(Ez(1:imax/4), 'LineWidth', 2);
title('Ez on the left of the source');
