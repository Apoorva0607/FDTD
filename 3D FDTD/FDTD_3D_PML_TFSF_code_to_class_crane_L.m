...
% every time step). 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              % NEW Code segment to account for inductance 

Ca = 1.0;
Cb = dt / (epso*delta);
L = 50*1E-6; % inductance of lumped element
C_L = (dt^2)/(epso*L*delta); % constant for extra term while the lumped element is inductor
Ez_sum = 0;% for the extra term when lumped element is inductor
...
Cb_z(imax/2,jmax/2,2:61) = (dt/(epso*delta))/(1+((sigma*dt)/(2*epso))); % crane conuctivity update for Ca an Cb
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          % new code segment to account for inductance as lumped element at
          % the base of the crane
    
    Ez_sum = Ez_sum + Ez(imax/2,jmax/2,1);
    Ez(imax/2,jmax/2,1) = Ez(imax/2,jmax/2,1) - C_L*Ez_sum;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % regular Ez update
